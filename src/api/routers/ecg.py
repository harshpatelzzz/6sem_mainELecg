...
async def analyze_ecg(
    ...
    print("FILE OBJECT:", ecg_file)
    print("FILENAME:", ecg_file.filename if ecg_file else "NO FILE")
    ...
    try:
        ...
        if len(contents) == 0:
            return Envelope(status="error", data={}, message="Empty file uploaded")
        ...
        if numeric_df.empty:
            return Envelope(status="error", data={}, message="No numeric ECG data found")

        signal = numeric_df.values.flatten().astype(np.float32).tolist()
        print("SIGNAL LENGTH:", len(signal))
        print("RETURNING SIGNAL SAMPLE:", signal[:10])

        # Perform inference on first beat
        beats=seg.segment_beats(filt.full_filter_pipeline(np.array(signal,dtype=np.float32))[0], seg.detect_r_peaks(filt.full_filter_pipeline(np.array(signal,dtype=np.float32))[0])); beats=beats if beats.shape[0] else np.zeros((1,216),dtype=np.float32)
        graph=gb.build_graph(beats, seg.compute_rri(seg.detect_r_peaks(filt.full_filter_pipeline(np.array(signal,dtype=np.float32))[0]))); out=model(torch.tensor(beats[:1],dtype=torch.float32).unsqueeze(1), Batch.from_data_list([graph]), [patient_id])
        ...
        payload=AnalysisResponse(patient_id=patient_id,timestamp=datetime.utcnow(),predicted_class=str(int(pred.item())),confidence=float(conf.item()),is_ood=bool(float(conf.item())<0.5),anomaly_score=float(a.item()),deviation_score=float(d.item()),uncertainty=float(u),risk_score=float(rs),risk_level=rl,alert=alert,risk_trajectory=traj,ecg_signal=signal)
        
        # Persistence
        if patient_id not in PATIENT_DB:
            PATIENT_DB[patient_id] = {"info": {"id": patient_id, "name": "Unknown", "age": 0}, "history": []}
        PATIENT_DB[patient_id]["history"].append(payload.model_dump())
        
        return Envelope(status="ok", data=payload.model_dump(), message="analysis complete")
    except Exception as e:
        print("ERROR:", str(e))
        return Envelope(status="error", data={}, message=str(e))
